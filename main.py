...
python main.py --config config/simple.yaml --target imgs/portrait.png
"""
...
from primitive import primitive, render_shapes_mixture, render_shapes_reinforced_ddepth, background
...
if __name__ == "__main__":
    ...
    #     primitive_list0.append(prim.from_instance())
    ...
    if cfg.save.loss:
        ...
        plt.clf()
        plt.plot(range(cfg.num_iter), number_record, linestyle='-')
        plt.title('Number of primitives over iterations')
        plt.xlabel('Iterations')
        plt.ylabel('Number of primitives')
        #plt.grid(True)
        filename = os.path.join(cfg.experiment_dir, f"number_for_{cfg.num_iter}iters_reorder.png")
        plt.savefig(filename)